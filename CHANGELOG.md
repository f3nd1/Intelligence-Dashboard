...
## v1.9.8-c531-migrate (2026-07-21)

- Migrated section 5.3.1 (Partnerships) compute to the server. Extracted the interleaved c531 compute (agreement lifecycle/expiry windows, NDA state, monitoring and evaluation over the monitoring_childtable/table_luoo child tables, selection-score thresholds, signature and risk field probing, monitoring recency, missing-controls and quality completeness) into a clean `buildC531()` (verified byte-identical to HEAD in-harness). Ported to `compute_c531` (RestrictedPython-safe), proven byte-identical to the JavaScript across 3 fixtures including a no-rating case. Added `c531_analytics` which resolves the Provider/Supplier Rating source with **frappe.db.exists first** so a missing DocType can never raise the recurring "Provider Rating not found" error again (the permanent, audit-wide fix now also covers this new action). Wired the client with `buildC531()` fallback. Verified in-browser: server path drives KPIs/charts/records, both paths render with zero console errors; no C5 regressions.


## v1.9.8-c522-migrate (2026-07-21)

- Migrated section 5.2.2 (Course Delivery) compute to the server, same pattern. Extracted the largest section's interleaved compute (20 likert rating fields with numericRating scaling, observation coverage, sign-off state and aging, rating bands, strengths/improvement text probing, survey-category and survey-volume grouping over the response child table, delivery exceptions) into a clean `buildC522()`. To avoid toFixed rounding-parity risk, rating averages are returned RAW and the render branch applies toFixed, so the server and fallback paths format identically in the browser (verified c522 renders byte-identical to HEAD in-harness). Ported to `compute_c522` (RestrictedPython-safe, name-based dedup instead of id()), proven byte-identical to the JavaScript across 3 fixtures (numeric-normalized). Added `c522_analytics` (Classroom Observation / Module Class Details / Survey Response full docs, Survey Response programme-filtered; Course Schedule and Student Attendance left empty to match the client's c522 branch, which does not load them). Wired the client with `buildC522()` fallback. Verified in-browser: server path drives KPIs (e.g. score 3.7/5) and charts, both paths render with zero console errors; no C5 regressions.
