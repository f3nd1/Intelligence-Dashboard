/* =========================================================
   UCC INTELLIGENCE PLATFORM v1.7.0
   Deployment target: one Frappe Custom HTML Block JavaScript field.
   ========================================================= */
...
(function () {
  ...
  function setWorkspace(workspace) {
    ...
    if (dashboardControl) {
      dashboardControl.hidden = workspace !== "analytics";
    }

    if (status) {
      status.textContent = workspace === "analytics"
        ? "Criterion 5 is active. Criterion 4 is also available; Criteria 1–3 and 6–7 remain planned."
        : "Ask UCC is active. Guided questions work without OpenAI; free-form AI is optional.";
    }
  }
  ...
  const shellHome = shell ? document.createComment("ucc-platform-shell-home") : null;
  const floatingShellButton = document.createElement("button");
  let shellPinnedOpen = false;

  floatingShellButton.type = "button";
  floatingShellButton.className = "ucc-shell-floating-launcher";
  floatingShellButton.textContent = "UCC";
  floatingShellButton.setAttribute("aria-label", "Open UCC navigation");
  floatingShellButton.setAttribute("title", "Open UCC navigation");
  floatingShellButton.hidden = true;

  if (document.body) {
    document.body.appendChild(floatingShellButton);
  }

  if (shell && shell.parentNode && shellHome) {
    shell.parentNode.insertBefore(shellHome, shell);
  }

  const shellCssVariables = [
    "--ucc-blue", "--ucc-blue-dark", "--ucc-blue-mid", "--ucc-blue-soft",
    "--ucc-gold", "--ucc-gold-dark", "--ucc-gold-soft",
    "--ucc-canvas", "--ucc-surface", "--ucc-text", "--ucc-muted",
    "--ucc-line", "--ucc-shadow"
  ];

  function copyShellThemeVariables() {
    if (!shell || !root || typeof getComputedStyle !== "function") return;
    const computed = getComputedStyle(root);
    shellCssVariables.forEach(function (name) {
      const value = computed.getPropertyValue(name);
      if (value) shell.style.setProperty(name, value.trim());
    });
  }

  function moveShellToViewport() {
    if (!shell || !document.body || shell.parentNode === document.body) return;
    copyShellThemeVariables();
    shell.classList.add("is-ported");
    document.body.appendChild(shell);
  }

  function restoreShellToPlatform() {
    if (!shell || !shellHome || !shellHome.parentNode) return;
    shellHome.parentNode.insertBefore(shell, shellHome.nextSibling);
    shell.classList.remove("is-ported");
  }

  function setDashboard(dashboard) {
    dashboardPanels.forEach(function (panel) {
      panel.classList.toggle("ucc-hidden", panel.dataset.dashboardPanel !== dashboard);
    });
    if (dashboardSelect && dashboardSelect.value !== dashboard) {
      dashboardSelect.value = dashboard;
    }
    if (status) {
      status.textContent = dashboard === "criterion_4"
        ? "Criterion 4 is active. Live DocTypes and fields are resolved at runtime for the current user."
        : "Criterion 5 is active. Ask UCC remains optional and guided questions work without OpenAI.";
    }
  ...
  if (dashboardSelect) {
    dashboardSelect.addEventListener("change", function () {
      setDashboard(dashboardSelect.value);
    });
  }

  const shellScrollTargets = [];
  let shellFrame = 0;

  function collectShellScrollTargets() {
    shellScrollTargets.length = 0;
    let node = root.parentElement;

    while (node && node !== document.body && node !== document.documentElement) {
      try {
        const style = getComputedStyle(node);
        const overflow = [style.overflow, style.overflowY].join(" ");
        if (/(auto|scroll|overlay)/i.test(overflow)) {
          shellScrollTargets.push(node);
        }
      } catch (error) {}
  ...
  function shellHasScrolled() {
    ...
    shellScrollTargets.forEach(function (target) {
      containerScroll = Math.max(containerScroll, Number(target.scrollTop || 0));
    });

    let rootPassed = false;
    try {
      rootPassed = root.getBoundingClientRect().top < -120;
    } catch (error) {}

    return documentScroll > 150 || containerScroll > 150 || rootPassed;
  }

  function updateShellMode() {
    if (!shell) return;
    const compact = shellHasScrolled();

    if (!compact) {
      shellPinnedOpen = false;
      restoreShellToPlatform();
      shell.classList.remove("is-compact", "is-expanded");
      floatingShellButton.hidden = true;
      return;
    }
    ...
    if (shellPinnedOpen) {
      moveShellToViewport();
      shell.classList.add("is-compact", "is-expanded");
      floatingShellButton.hidden = true;
    } else {
      // Keep the full shell in its original page position. It can scroll away
      // naturally while the independent launcher remains visible.
      restoreShellToPlatform();
      shell.classList.remove("is-compact", "is-expanded");
      floatingShellButton.hidden = false;
    }
  }

  function scheduleShellMode() {
    if (shellFrame) return;
    shellFrame = window.requestAnimationFrame(function () {
      shellFrame = 0;
      updateShellMode();
    });
  }

  floatingShellButton.addEventListener("click", function () {
    shellPinnedOpen = true;
    updateShellMode();
  });

  if (shellToggle) {
    shellToggle.addEventListener("click", function () {
      shellPinnedOpen = !shellPinnedOpen;
      updateShellMode();
    });
  }

  collectShellScrollTargets();
  shellScrollTargets.forEach(function (target) {
    target.addEventListener("scroll", scheduleShellMode, { passive: true });
  });
  document.addEventListener("scroll", scheduleShellMode, true);
  window.addEventListener("scroll", scheduleShellMode, { passive: true });
  ...
  if (!["criterion_4", "criterion_5"].includes(savedDashboard)) savedDashboard = "criterion_5";
...
/* ===== CRITERION 5 SOURCE JAVASCRIPT v5.6.1 ===== */

(function(){
 const BUILD_ID="UCC-C5-5.6.1-20260718";
 ...
 function setC5Notice(message,noticeStatus="available"){
  const notice=$("[data-c5-source-notice]");
  if(!notice)return;
  if(notice.dataset.dismissed==="1"&&noticeStatus==="available")return;
  if(noticeStatus!=="available")notice.dataset.dismissed="0";
  notice.hidden=false;
  notice.dataset.status=noticeStatus;
  const strong=notice.querySelector("strong");
  const copy=notice.querySelector("span");
  if(strong){
   strong.textContent=noticeStatus==="available"
    ?"Criterion 5 live analytics active."
    :noticeStatus==="loading"
      ?"Loading Criterion 5 analytics…"
      :"Criterion 5 data notice.";
  }
 ...
 function renderC511(){
  ...
  // Charts render separately after the active panel is visible.
  ...
  const qaBody=$('[data-table="c511-qa"]');if(qaBody)qaBody.innerHTML=questions.map(row=>`<tr><td>${row[0]}</td><td>${esc(row[1])}</td><td>${esc(row[2])}</td><td>${sourceCell(row[3])}</td><td>${badge(row[4])}</td></tr>`).join("");
 ...
 function renderOverviewQA(){
  ...
  body.innerHTML=rows.length?rows.map((row,index)=>`<tr>
    <td>${esc(row.criterion)}</td>
    <td>${esc(row.question)}</td>
    <td>${esc(row.answer)}${row.records?.length?`<br><button type="button" class="record-link" data-overview-qa="${index}">View ${row.records.length} matching record(s) ↗</button>`:""}</td>
    <td>${sourceCell(row.source,row.doctypes)}</td>
    <td>${badge(row.status)}</td>
  </tr>`).join(""):'<tr><td colspan="5">No questions for the selected criterion.</td></tr>';
  body.querySelectorAll("[data-overview-qa]").forEach(button=>button.addEventListener("click",()=>{
    ...
    openAffected(row.question,row.doctype,row.records);
  }));
 }
 function renderQATable(key,rows){
  ...
  body.innerHTML=rows.length?rows.map((row,index)=>`<tr>
    <td>${esc(row.criterion)}</td>
    <td>${esc(row.question)}</td>
    <td>${esc(row.answer)}${row.records?.length?`<br><button type="button" class="record-link" data-qa-index="${index}">View ${row.records.length} matching record(s) ↗</button>`:""}</td>
    <td>${sourceCell(row.source,row.doctypes)}</td>
    <td>${badge(row.status)}</td>
  </tr>`).join(""):'<tr><td colspan="5">No questions available for this criterion.</td></tr>';
...
if(tab==="c53"){
    // No confirmed partnership source is available.
  }
 ...
 async function loadSection(tab, force=false){
  ...
  try{
    ...
    if(tab==="overview"){
      // Overview intentionally stays lightweight.
    }
    ...
    if(tab==="c54"){
      ...
      // Attendance supports learning-risk indicators, but loads only in 5.4.
      const cs=[["Course Schedule","schedule_date",">=",f.start],["Course Schedule","schedule_date","<=",f.end]];
      if(f.program)cs.push(["Course Schedule","program","=",f.program]);
      if(f.student_group)cs.push(["Course Schedule","student_group","=",f.student_group]);
    ...
    if(tab==="quality"){
      // Load all core operational sources needed for quality checks.
      const cs=[["Course Schedule","schedule_date",">=",f.start],["Course Schedule","schedule_date","<=",f.end]];
      if(f.program)cs.push(["Course Schedule","program","=",f.program]);
      if(f.student_group)cs.push(["Course Schedule","student_group","=",f.student_group]);
    ...
    // Load attendance only after schedules are available.
 ...
 function renderSubnav(parent,active){
  const nav=$("[data-subnav]"),children=SECTION_REGISTRY[parent]?.children||[];
  if(!nav)return;
  if(!children.length){nav.classList.add("hidden");nav.innerHTML="";return}
  nav.innerHTML=children.map(item=>`<button type="button" data-section="${item.id}" class="${item.id===active?"active":""}">${esc(item.label)}</button>`).join("");
  nav.querySelectorAll("[data-section]").forEach(button=>button.addEventListener("click",()=>{
    showTab(button.dataset.section);
    nav.classList.add("hidden");
  }));
  const parentButton=$(`[data-tab="${parent}"]`);
  if(parentButton){
    const tabs=parentButton.closest(".tabs");
    if(tabs){
      if(nav.parentElement!==tabs)tabs.appendChild(nav);
      nav.style.left=`${parentButton.offsetLeft}px`;
      nav.style.top=`${parentButton.offsetTop+parentButton.offsetHeight+6}px`;
    }
    const open=()=>nav.classList.remove("hidden");
    const closeLater=()=>setTimeout(()=>{
      if(!nav.matches(":hover")&&!parentButton.matches(":hover"))nav.classList.add("hidden");
    },120);
    parentButton.onmouseenter=open;
    parentButton.onfocus=open;
    parentButton.onmouseleave=closeLater;
    nav.onmouseleave=closeLater;
    nav.onmouseenter=open;
  }
 }
 function showTab(tab){
  const parent=parentTab(tab);root.dataset.activeTab=tab;
  $$("[data-tab]").forEach(b=>b.classList.toggle("active",b.dataset.tab===parent));
  $$("[data-panel]").forEach(p=>p.classList.toggle("hidden",p.dataset.panel!==tab));
  renderSubnav(parent,tab);loadSection(tab);
 }
 function drill(kind){
  let rows=[],cols=[];
  const d=state.data;
...
/* ===== ASK UCC · UCC INTELLIGENCE PLATFORM v1.2.0 ===== */
(function () {
  "use strict";
  ...
  async function sendQuestion(question, selectedApplicant, showUserMessage) {
    ...
    try {
      ...
      try {
        ...
        // Session storage is optional. Deterministic ERPNext checks still work.
      }
...
/* =========================================================
   UCC INTELLIGENCE PLATFORM v1.2.0 UNIVERSAL INTERACTION LAYER
   ========================================================= */
(function(){
  "use strict";
  const root=typeof root_element!=="undefined"
  ...
  function csvCell(value){return '"'+String(value??"").replace(/"/g,'""').replace(/\s+/g," ").trim()+'"'}
  function tableToCsv(table){
    return $$("tr",table).map(row=>$$("th,td",row).map(cell=>csvCell(cell.innerText)).join(",")).join("\n");
  }
...
/* ===== CRITERION 4 ANALYTICS HUB v1.7.0 ===== */
(function(){
  "use strict";
